...
#ifndef SPS_JOIN_HPP_
#define SPS_JOIN_HPP_
struct sps_join : public record {
	...
	static sps_join* deserialize( const char* const stream) {
		std::size_t bytes_read = 0;
		sps_join* s = new sps_join();
	...
	static std::string serialize( const sps_join* const s) {
		std::stringstream ss;
